...
from m46mneg2_ratio_plot import compute_ratio_stats
from rempf.io.npz import load_npz, save_npz


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare E[M4] / (E[M6] E[M_{-2}]) ratio curves on common axes."
    )
...
def _plot_curve(ax: plt.Axes, stats: dict[str, np.ndarray], *, label: str, error_bars: str) -> None:
...
def main() -> None:
    ...
    n_union = np.unique(
        np.concatenate((np.asarray(stats_a["n_list"], dtype=int), np.asarray(stats_b["n_list"], dtype=int)))
    )
