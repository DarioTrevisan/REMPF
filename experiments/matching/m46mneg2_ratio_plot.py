...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot E[M4] / (E[M6] E[M_{-2}]) for matching sweeps."
    )
    parser.add_argument("npz", type=str, help="Sweep output with q_eval containing -2, 4, and 6")
...
def compute_ratio_stats(data: dict) -> dict[str, np.ndarray]:
    q_eval = np.asarray(data["q_eval"], dtype=float)
    costs = np.asarray(data["costs"], dtype=float)
    n_list = np.asarray(data["n_list"], dtype=int)

    try:
        qneg2_idx = int(np.where(np.isclose(q_eval, -2.0))[0][0])
        q4_idx = int(np.where(np.isclose(q_eval, 4.0))[0][0])
        q6_idx = int(np.where(np.isclose(q_eval, 6.0))[0][0])
    except IndexError as exc:
        raise ValueError("Input sweep must contain q=-2, q=4, and q=6 in q_eval.") from exc

    neg_second_moments = costs[:, :, qneg2_idx] / n_list[:, None]
    fourth_moments = costs[:, :, q4_idx] / n_list[:, None]
    sixth_moments = costs[:, :, q6_idx] / n_list[:, None]

    mean_neg_second_moment = neg_second_moments.mean(axis=1)
    mean_fourth_moment = fourth_moments.mean(axis=1)
    mean_sixth_moment = sixth_moments.mean(axis=1)
    ratio = mean_fourth_moment / (mean_sixth_moment * mean_neg_second_moment)

    ratio_se = np.full_like(ratio, np.nan, dtype=float)
    trials = neg_second_moments.shape[1]
    if trials > 1:
        for i in range(len(n_list)):
            sample = np.column_stack((neg_second_moments[i], fourth_moments[i], sixth_moments[i]))
            cov = np.cov(sample, rowvar=False, ddof=1)
            a = mean_neg_second_moment[i]
            b = mean_fourth_moment[i]
            c = mean_sixth_moment[i]
            grad = np.array(
                [
                    -b / (c * a**2),
                    1.0 / (c * a),
    ...
    return {
        "n_list": n_list,
        "q_eval": q_eval,
        "mean_cost_qneg2": costs[:, :, qneg2_idx].mean(axis=1),
        "mean_cost_q4": costs[:, :, q4_idx].mean(axis=1),
        "mean_cost_q6": costs[:, :, q6_idx].mean(axis=1),
        "mean_neg_second_moment": mean_neg_second_moment,
        "mean_fourth_moment": mean_fourth_moment,
        "mean_sixth_moment": mean_sixth_moment,
        "ratio": ratio,
        "ratio_se": ratio_se,
    }
...
def main() -> None:
    ...
    ax.set_xlabel("n")
    ax.set_ylabel(r"$\mathbb{E}[M_4] / (\mathbb{E}[M_6]\mathbb{E}[M_{-2}])$")
    ax.set_title(
    ...
    params = {
        "problem": "matching_m46mneg2_ratio",
        ...
        "observable": "E[(1/n) sum |X_i-Y_sigma(i)|^4] / (E[(1/n) sum |X_i-Y_sigma(i)|^6] E[(1/n) sum |X_i-Y_sigma(i)|^{-2}])",
    }
    save_npz(
        str(out_data),
        arrays={
            "n_list": n_list,
            "mean_cost_qneg2": np.asarray(stats["mean_cost_qneg2"], dtype=float),
            "mean_cost_q4": np.asarray(stats["mean_cost_q4"], dtype=float),
            "mean_cost_q6": np.asarray(stats["mean_cost_q6"], dtype=float),
            "mean_neg_second_moment": np.asarray(stats["mean_neg_second_moment"], dtype=float),
            "mean_fourth_moment": np.asarray(stats["mean_fourth_moment"], dtype=float),
            "mean_sixth_moment": np.asarray(stats["mean_sixth_moment"], dtype=float),
            "ratio": ratio,
            "ratio_se": ratio_se,
        },
        params=params,
